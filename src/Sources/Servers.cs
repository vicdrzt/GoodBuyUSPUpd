...
namespace GoodbyeUsp.Sources
{
    internal class Servers
    {
        ...
        public static string SelectConsumptionsQuery = @"SELECT * FROM (
                ...
                AND b.ServiceProviderId=1 AND bd.PeriodStartDate>='1.1.19' AND b.PAYMENTTYPECODE = 0 
        ...
        public static string SelectCitizensQuery = @"SELECT * FROM (
                SELECT r.code ClientId, p.NAME, p.SURNAME fam, p.PATRONIMIC otch, 1 vlad, TRIM(p.TAXCODE) TaxCode, 
                  ...
                  WHERE ac.RecordpointId = r.CODE AND ac.ContractId = ch.Id AND ch.PersonId = p.CODE AND r.ServiceProviderId=1 AND COALESCE(r.ISTECHNICAL,0)!=1
        ...
        public static string SelectPaymentsQuery = @"select clientId, summpay, datebank from
                (
                SELECT recordpointId clientId, SUM(pd.sum) summpay, pd.PayDate datebank
                FROM PaymentDocument pd
                  ...
                  join PAYMENTCHANNEL p on p.Code = pb.PaymentChannelId
                                ...
                                AND COALESCE(p.TypeCode,0) IN(0,2)
                                AND p.Name NOT LIKE '%Постанова КМУ № 64%' AND StatusId = 1
                                --and pd.RecordpointId in (select code from recordpoint where name in ('06-9-491/154'))
                                GROUP BY recordpointId, pd.PayDate HAVING SUM(pd.sum)>0       
                union all
                SELECT recordpointId clientId, SUM(pd.sum) summpay, pd.PayDate datebank
                FROM PaymentDocument pd
                ...
                left join PAYMENTCHANNEL p on p.Code = pb.PaymentChannelId
                               WHERE
                                COALESCE(p.TypeCode,0) IN(0,2)
                                AND StatusId = 1
                                and pd.TypeId=5 
                                --and pd.RecordpointId in (select code from recordpoint where name in ('06-9-491/154'))
                                GROUP BY pd.recordpointId, pd.PayDate
                                HAVING SUM(pd.sum)!=0) t";
        ...
        public static string SelectSubsidyQuery = @"SELECT ClientId, Datebank, SUM(Summpay) Summpay FROM (
                ...
                WHERE b.INVSTATUSCODE<>2 AND b.ServiceProviderId=1 AND COALESCE(b.SUBSIDY,0)<>0 
                UNION ALL SELECT pd.RecordpointId, pd.sum, CAST(PayDate AS DATETIME2) FROM PaymentBatch pb, PaymentDocument pd, PAYMENTCHANNEL p WHERE pb.id = pd.BatchId AND pb.ServiceProviderId=1 
        ...
        public const string SelectPay375Query = @"SELECT recordpointId clientId, SUM(pd.sum) summpay, pd.PayDate datebank FROM PaymentBatch pb, PaymentDocument pd, PAYMENTCHANNEL p 
                WHERE pb.id = pd.BatchId AND pb.ServiceProviderId=1 
                AND  p.Code = pb.PaymentChannelId AND p.TypeCode=1 AND StatusId=1 GROUP BY recordpointId, pd.PayDate HAVING SUM(pd.sum)>0";
        ...
        public const string SelectPay64Query = @"SELECT recordpointId clientId, SUM(pd.sum) summpay, pd.PayDate datebank FROM PaymentBatch pb, PaymentDocument pd, PAYMENTCHANNEL p 
                WHERE pb.id = pd.BatchId AND pb.ServiceProviderId=1 AND  p.Code = pb.PaymentChannelId AND StatusId=1 AND p.Name LIKE '%Постанова КМУ № 64%' GROUP BY recordpointId, pd.PayDate";
        ...
        public const string SelectMonetizedDiscounts = @"SELECT pd.RecordpointId ClientId, SUM(pd.sum) summpay, PayDate Datebank FROM PaymentBatch pb, PaymentDocument pd, PAYMENTCHANNEL p WHERE pb.id = pd.BatchId 
                AND pb.ServiceProviderId=1 AND pd.sum<>0 AND p.Name LIKE '%Пільга%' AND Typecode=3 AND p.Code = pb.PaymentChannelId AND StatusId=1 GROUP BY pd.RecordpointId, pd.PayDate";
